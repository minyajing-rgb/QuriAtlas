...
with sync_playwright() as pw:
    ...
    check('Three live HUD cards',page.locator('.hero-hud').count()==3)
    ...
    check('Mobile hero remains visual',page.locator('.hero-art').bounding_box()['height']>=360)
    page.screenshot(path=str(out/'mobile-zh.png'),full_page=False)
    check('No JS errors',not errors)
    browser.close()
report={'build':'0.4.0-atlantis-cinematic','passed':len(checks),'checks':checks,'javascript_errors':errors,'url':a.url}
